...
class FloatingText:
    def __init__(self, x: float, y: float, text: str, color: Tuple[int, int, int], size: int = 24, is_tetris: bool = False, combo_count: int = 0):
        ...
        self.shake_offset = 3 if is_tetris else 0
    ...
    def update(self):
        ...
        if self.appear_time > 0:
            ...
            if self.combo_count > 0:
                shake_intensity = min(4, self.combo_count * 0.8)
    ...
    def draw(self, surface: pygame.Surface, font: pygame.font.Font, font_name: str = None):
        ...
        if font_name:
            scaled_font = pygame.font.SysFont(font_name, new_size)
        else:
...
class Game:
    ...
    def draw_neon_text(self, surface: pygame.Surface, text: str, font: pygame.font.Font, 
        ...
        for i in range(glow_size, 0, -1):
            ...
            for dx in range(-i, i + 1):
                for dy in range(-i, i + 1):
                    if dx * dx + dy * dy <= i * i:
                        temp_surf = font.render(text, True, (*color, alpha))
                        glow_surf.blit(temp_surf, (10 + dx, 10 + dy))
        surface.blit(glow_surf, (x - 10, y - 10))
        
        outline_color = (min(255, color[0] + 60), min(255, color[1] + 60), min(255, color[2] + 60))
        for dx in [-1, 0, 1]:
            for dy in [-1, 0, 1]:
                if dx != 0 or dy != 0:
    ...
    def draw_panel(self):
        ...
        title_x = PANEL_X + (PANEL_WIDTH - 150) // 2
        self.draw_neon_text(self.screen, "TETRIS", self.font_large, title_x, BOARD_Y + 10, (0, 255, 255), 4, True)
    ...
    def draw_start_screen(self):
        ...
        self.draw_neon_text(self.screen, "TETRIS", self.font_large, (SCREEN_WIDTH - 150) // 2, 150, (0, 255, 255), 4, True)
